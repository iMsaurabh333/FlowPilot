...
- Health states are `never_checked`, `healthy`, `unhealthy`, and `stale`. An enabled server becomes eligible only after a successful authenticated probe; that confirmed status is retained across API restarts so scheduled collection can start immediately. New, disabled, and failed servers contribute no tools, while a stale status remains an administrator prompt to revalidate the connection.
