...
Latest clean-clone rehearsal (2026-09-01, pushed commit `8180e35`) rebuilt the same MTAR from a new GitHub checkout after restoring every lockfile. The strict build exited `0` with a `34,130,048`-byte archive (`SHA-256 4E2CB32C3CC174DDBB3B607D0669B5EBFDF92CFDE3DD3B7A81EEC329B3CC4191`). Its nested payloads contained 11,921 API files and 5,266 AppRouter files; required runtime/package entries were present, 263 AppRouter assets were present, stale hashed assets were absent, and the prohibited-entry count was zero. The first bounded attempt timed out in MBT's web `npm ci` because its subprocess used the shared npm cache; setting `NPM_CONFIG_CACHE` to the clone-local cache made the single retry succeed. The live `btp:bootstrap --mode verify` check was run read-only but failed closed because the browser SSO session had expired; refresh SSO manually before relying on live target verification.
