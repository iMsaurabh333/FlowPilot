...
type SavedJob = { id: string; name: string; savedAt: string; artifacts: Artifact[] };
...
export function BulkActionsView({ client }: { client: FlowPilotApi }) {
  ...
  const saveJob = () => {
    ...
    const local: SavedJob = { id: crypto.randomUUID(), name: jobName.trim(), savedAt: new Date().toISOString(), artifacts };
    if (client.createBulkJob) { void client.createBulkJob({ title: local.name, artifacts: local.artifacts }).then((created) => { const saved = { id: created.id, name: created.title, savedAt: created.createdAt, artifacts: created.artifacts as Artifact[] }; setJobs((items) => [saved, ...items]); setActiveJob(saved); setMode("jobs"); setNotice(`Saved ${saved.name}.`); }).catch(() => setNotice("The bulk job could not be saved.")); return; }
  ...
  const newJob = () => { setMode("wizard"); setStep(1); setSelectedPackages([]); setArtifacts([]); setJobName(""); setNotice(undefined); };
