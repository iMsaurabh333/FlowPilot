...
export class BulkJobStore {
  ...
  update(user: AuthenticatedUser, jobId: string, title: string, artifacts: unknown[], scheduledFor: Date | null) { return this.#identity(user, async (client) => { const row = (await client.query<Row>(`UPDATE ${this.#table} SET title=$2,artifacts=$3,scheduled_for=$4,schedule_active=true,scheduler_job_id=NULL,updated_at=now() WHERE id=$1 RETURNING *`, [jobId, title, JSON.stringify(artifacts), scheduledFor])).rows[0]; return row ? this.#map(row) : undefined; }); }
  remove(user: AuthenticatedUser, jobId: string) { return this.#identity(user, async (client) => (await client.query(`DELETE FROM ${this.#table} WHERE id=$1`, [jobId])).rowCount === 1); }
