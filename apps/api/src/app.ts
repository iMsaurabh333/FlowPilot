...
import {
  ActionPlanImmutableError,
  ActionPlanNotFoundError,
  type PostgresActionPlanStore,
} from "./reports/action-plan-store.js";
...
} from "./reports/reconciliation.js";
...
const conversationPolicySchema = z
  ...
  .strict();
const integrationHealthSettingsSchema = z
...
const reportJobInputSchema = z
  ...
  .strict();
const reconciliationUploadSchema = z
  .object({
...
const reconciliationRunSchema = z
  ...
  .strict();
const reconciliationExportSchema = z
  .object({
  ...
  .strict();
const contentFlowSchema = z
  .object({
    integrationFlowId: z.string().trim().min(1).max(256),
    version: z.string().trim().min(1).max(256),
  })
  .strict();
const contentConfigurationSchema = contentFlowSchema
  .extend({
    configuration: z
      .record(
        ...
        z
          .object({
            ...
            dataType: z.string().trim().min(1).max(100),
          })
          .strict(),
      )
      ...
      .array(
        z
          .object({
            ...
            action: z.enum(["deploy", "undeploy", "none"]),
            configure: z.boolean(),
            parameters: z
              .array(
                z
...
export function createApp(options: AppOptions) {
  ...
  if (options.reports && options.schedulerAuthentication) {
    app.post(
      "/internal/reports/dispatch",
      options.schedulerAuthentication,
      async (request, response, next) => {
        try {
          const body = z
            .object({ reportJobId: z.string().uuid() })
            .strict()
            .parse(request.body);
  ...
  if (options.bulkJobs && options.schedulerAuthentication)
    app.post(
      "/internal/bulk-actions/dispatch",
      options.schedulerAuthentication,
      async (request, response, next) => {
        try {
          const job = await options.bulkJobs!.findScheduled(
            z.object({ reportJobId: z.string().uuid() }).parse(request.body)
              .reportJobId,
          );
          if (!job) return response.status(404).end();
          await executeBulk(job.user, job.artifacts);
          response.status(204).end();
        } catch (error) {
          next(error);
        }
  ...
  const invokeContent = async (
    ...
    const tool = (await options.contentTools(user)).find((candidate) =>
      candidate.name.endsWith(`__${name}`),
    );
    ...
    try {
      ...
      if (typeof parsed === "object" && parsed !== null && "error" in parsed)
        throw new Error("content_tool_failed");
      return parsed;
    } catch (error) {
      if (error instanceof Error && error.message === "content_tool_failed")
        throw error;
      throw new Error("content_tool_invalid_response");
    }
  };
  async function executeBulk(
    ...
    for (const artifact of artifacts as Array<{
      id: string;
      version: string;
      action: "deploy" | "undeploy" | "none";
      configure: boolean;
      parameters: Array<{ key: string; value: string; dataType: string }>;
    }>) {
      if (artifact.action === "none") continue;
      if (artifact.configure && artifact.parameters.length)
        await invokeContent(user, "update_integration_flow_configuration", {
          integrationFlowId: artifact.id,
          version: artifact.version,
          configuration: Object.fromEntries(
            artifact.parameters.map((p) => [
              p.key,
              { value: p.value, dataType: p.dataType },
            ]),
          ),
        });
      await invokeContent(
        user,
        artifact.action === "deploy"
          ? "deploy_integration_flow"
          : "undeploy_integration_flow",
        artifact.action === "deploy"
          ? { integrationFlowId: artifact.id, version: artifact.version }
          : { integrationFlowId: artifact.id },
      );
    }
  }
  ...
  app.get("/api/reports/jobs", async (request, response, next) => {
    ...
    try {
      response
        .status(200)
        .json({ jobs: await options.reports.list(authenticatedUser(request)) });
    } catch (error) {
      next(error);
    }
  });
  app.get("/api/operation-logs", async (request, response, next) => {
    if (!options.operationLogs) {
      response.status(503).json({ error: "logs_unavailable" });
      return;
    }
    try {
      response
        .status(200)
        .json({
          logs: await options.operationLogs.list(authenticatedUser(request)),
        });
    } catch (error) {
      next(error);
    }
  ...
  app.get(
    ...
    async (request, response, next) => {
      if (!options.integrationHealth)
        return response
          .status(503)
          .json({ error: "integration_health_unavailable" });
      try {
        const window = z
          .enum(["last_completed_hour", "today", "yesterday", "last_24_hours"])
          .catch("last_completed_hour")
          .parse(request.query.window) as HealthSummary["window"];
        response
          .status(200)
          .json(
            await options.integrationHealth.summary(
              authenticatedUser(request),
              window,
            ),
          );
  ...
  // This endpoint is intentionally operator-only: it is useful for an initial
  // backfill and for scheduler wiring, but never changes CPI state.
  app.post(
    ...
    async (request, response, next) => {
      if (!options.integrationHealth)
        return response
          ...
          .json({ error: "integration_health_unavailable" });
      try {
        const hours = z.coerce.number().int().min(1).max(24).catch(1).parse(request.query.hours);
        response.status(202).json(await options.integrationHealth.collectRecent(authenticatedUser(request), hours));
      } catch (error) {
        next(error);
      }
    },
    ...
    async (request, response, next) => {
      ...
      try {
        response
          .status(200)
          .json({
            sources: await options.reportSources(authenticatedUser(request)),
          });
      } catch (error) {
        next(error);
      }
  ...
  app.get(
    ...
    async (request, response, next) => {
      if (!options.bulkJobs)
        return response.status(503).json({ error: "bulk_jobs_unavailable" });
      try {
        response.json({
          jobs: await options.bulkJobs.list(authenticatedUser(request)),
        });
      } catch (error) {
        next(error);
      }
  ...
  app.post(
    ...
    async (request, response, next) => {
      if (!options.bulkJobs)
        return response.status(503).json({ error: "bulk_jobs_unavailable" });
      try {
        const input = bulkJobSchema.parse(request.body);
        const scheduledFor = input.scheduledFor
          ? new Date(input.scheduledFor)
          : null;
        if (scheduledFor && scheduledFor.getTime() < Date.now() + 10 * 60_000)
          return response.status(400).json({ error: "schedule_time_too_soon" });
        const job = await options.bulkJobs.create(
          authenticatedUser(request),
          input.title,
          input.artifacts,
          scheduledFor,
        ...
        if (
          scheduledFor &&
          options.bulkScheduler &&
          options.bulkSchedulerActionUrl
        )
          await options.bulkJobs.setSchedulerJobId(
            authenticatedUser(request),
            job.id,
            await options.bulkScheduler.schedule({
              reportJobId: job.id,
              title: job.title,
              actionUrl: options.bulkSchedulerActionUrl,
              scheduledFor,
              recurrenceRule: null,
            }),
          );
        response
          .status(201)
          .json(
            await options.bulkJobs.findOwned(
              authenticatedUser(request),
              job.id,
            ),
          );
  ...
  app.post(
    "/api/bulk-actions/jobs/:jobId/run",
    reportOperator,
    async (request, response, next) => {
      if (!options.bulkJobs)
        return response.status(503).json({ error: "bulk_jobs_unavailable" });
      try {
        const job = await options.bulkJobs.findOwned(
          authenticatedUser(request),
          conversationIdSchema.parse(request.params.jobId),
        );
        if (!job) return response.status(404).json({ error: "not_found" });
        await executeBulk(authenticatedUser(request), job.artifacts);
        response.status(204).end();
      } catch (error) {
        next(error);
      }
  ...
  app.put("/api/bulk-actions/jobs/:jobId", reportOperator, async (request, response, next) => {
    if (!options.bulkJobs) return response.status(503).json({ error: "bulk_jobs_unavailable" });
    try {
      const user = authenticatedUser(request); const jobId = conversationIdSchema.parse(request.params.jobId); const input = bulkJobSchema.parse(request.body);
      const previous = await options.bulkJobs.findOwned(user, jobId); if (!previous) return response.status(404).json({ error: "not_found" });
      if (previous.schedulerJobId && options.bulkScheduler) await options.bulkScheduler.remove(previous.schedulerJobId);
      const scheduledFor = input.scheduledFor ? new Date(input.scheduledFor) : null;
      const updated = await options.bulkJobs.update(user, jobId, input.title, input.artifacts, scheduledFor); if (!updated) return response.status(404).json({ error: "not_found" });
      if (scheduledFor && options.bulkScheduler && options.bulkSchedulerActionUrl) await options.bulkJobs.setSchedulerJobId(user, jobId, await options.bulkScheduler.schedule({ reportJobId: jobId, title: updated.title, actionUrl: options.bulkSchedulerActionUrl, scheduledFor, recurrenceRule: null }));
      response.json(await options.bulkJobs.findOwned(user, jobId));
    } catch (error) { next(error); }
  });
  app.delete("/api/bulk-actions/jobs/:jobId", reportOperator, async (request, response, next) => {
    if (!options.bulkJobs) return response.status(503).json({ error: "bulk_jobs_unavailable" });
    try {
      const user = authenticatedUser(request); const jobId = conversationIdSchema.parse(request.params.jobId); const job = await options.bulkJobs.findOwned(user, jobId); if (!job) return response.status(404).json({ error: "not_found" });
      if (job.schedulerJobId && options.bulkScheduler) await options.bulkScheduler.remove(job.schedulerJobId);
      await options.bulkJobs.remove(user, jobId); response.status(204).end();
    } catch (error) { next(error); }
  });
  app.get(
    "/api/bulk-actions/packages/:packageId/flows",
    reportOperator,
    async (request, response, next) => {
      try {
        response.json(
          await invokeContent(
            authenticatedUser(request),
            "list_package_integration_flows",
            {
              packageId: z
                .string()
                .min(1)
                .max(256)
                .parse(request.params.packageId),
            },
          ),
        );
      } catch (error) {
  ...
  app.get(
    "/api/bulk-actions/flows/:flowId/configurations",
    reportOperator,
    async (request, response, next) => {
      try {
        const version = z.string().min(1).max(256).parse(request.query.version);
        response.json(
          await invokeContent(
            authenticatedUser(request),
            "list_integration_flow_configurations",
            {
              integrationFlowId: z
                .string()
                .min(1)
                .max(256)
                .parse(request.params.flowId),
              version,
            },
          ),
        );
  ...
  app.post(
    "/api/bulk-actions/flows/:flowId/deploy",
    reportOperator,
    async (request, response, next) => {
      try {
        const input = contentFlowSchema.parse({
          ...request.body,
          integrationFlowId: request.params.flowId,
        });
        response.json(
          await invokeContent(
            authenticatedUser(request),
  ...
  app.put(
    ...
    async (request, response, next) => {
      try {
        const input = contentConfigurationSchema.parse({
          ...request.body,
          integrationFlowId: request.params.flowId,
        });
        response.json(
          await invokeContent(
            authenticatedUser(request),
            "update_integration_flow_configuration",
            input,
          ),
        );
      } catch (error) {
  ...
  app.post(
    "/api/reconciliations/preview",
    reportOperator,
    async (request, response, next) => {
      try {
        const body = reconciliationUploadSchema.parse(request.body);
        response
          .status(200)
          .json(previewReconciliationUpload(body.fileName, body.contentBase64));
      } catch (error) {
        next(error);
      }
    },
  ...
  app.get(
    ...
    (_request, response) => {
      ...
      response.setHeader(
        "Content-Disposition",
        "attachment; filename=flowpilot-reconciliation-template.xlsx",
      );
  ...
  app.post(
    "/api/reconciliations/export",
    reportOperator,
    async (request, response, next) => {
      try {
        const result = reconciliationExportSchema.parse(request.body) as {
          generatedAt: string;
        ...
        response.setHeader(
          "Content-Disposition",
          "attachment; filename=flowpilot-reconciliation-report.xlsx",
        );
        response.status(200).send(reconciliationExport(result));
      } catch (error) {
        next(error);
      }
    },
      ...
      try {
        ...
        if (input.sourceToolNames?.length) {
          ...
          const available = new Set(
            (await options.reportSources(authenticatedUser(request))).map(
              (source) => source.name,
            ),
          );
          if (input.sourceToolNames.some((name) => !available.has(name))) {
        ...
        if (input.actionPlanId) {
          if (
            !options.actionPlans ||
            !(await options.actionPlans.findApproved(
              authenticatedUser(request),
              input.actionPlanId,
            ))
          ) {
  ...
  app.get(
    ...
    async (request, response, next) => {
      ...
      try {
        const format = z
          .enum(["html", "markdown", "xlsx"])
          .parse(request.query.format) as ReportExportFormat;
        const output = exportReport(
          await options.reports.get(
            authenticatedUser(request),
            conversationIdSchema.parse(request.params.jobId),
          ),
          format,
  ...
  app.get("/api/reports/jobs/:jobId/runs", async (request, response, next) => {
    ...
    try {
      response
        .status(200)
        .json({
          runs: await options.reports.listRuns(
            authenticatedUser(request),
            conversationIdSchema.parse(request.params.jobId),
          ),
        });
  ...
  app.get(
    "/api/reports/jobs/:jobId/runs/:runId/roadblock",
    async (request, response, next) => {
      if (!options.reports) {
        response.status(503).json({ error: "reports_unavailable" });
        return;
      ...
      try {
        const run = (
          await options.reports.listRuns(
            authenticatedUser(request),
            conversationIdSchema.parse(request.params.jobId),
          )
        ).find(
          (item) =>
            item.id === conversationIdSchema.parse(request.params.runId),
        );
        if (!run || !run.errorLog) {
        ...
        response.setHeader(
          "Content-Disposition",
          `attachment; filename="report-roadblock-${run.id}.txt"`,
        );
        response.send(run.errorLog);
      } catch (error) {
        next(error);
      }
    },
  ...
  app.post(
    "/api/reports/jobs/:jobId/schedule-active",
    reportOperator,
    async (request, response, next) => {
      if (!options.reports) {
        response.status(503).json({ error: "reports_unavailable" });
      ...
      try {
        ...
        response
          .status(200)
          .json(
            await options.reports.setScheduleActive(
              authenticatedUser(request),
              conversationIdSchema.parse(request.params.jobId),
              body.active,
            ),
  ...
  app.post(
    ...
    reportOperator,
    async (request, response, next) => {
      if (!options.reportPlanning || !options.actionPlans) {
        response.status(503).json({ error: "reports_unavailable" });
        return;
      }
      try {
        const body = z
          .object({
            fileName: z.string().trim().min(1).max(255),
            contentBase64: z.string().min(1).max(200_000),
          })
          .strict()
          .parse(request.body);
        ...
        response
          .status(201)
          .json(
            await options.actionPlans.create(
              authenticatedUser(request),
              source,
              preview.plan,
              preview.steps,
  ...
  app.put(
    "/api/reports/action-plans/:planId",
    reportOperator,
    async (request, response, next) => {
      if (!options.actionPlans) {
        response.status(503).json({ error: "reports_unavailable" });
        return;
      }
      try {
        const body = z
          .object({ plan: z.string().trim().min(1).max(60_000) })
          .strict()
          .parse(request.body);
        response
          .status(200)
          .json(
            await options.actionPlans.update(
              authenticatedUser(request),
              conversationIdSchema.parse(request.params.planId),
              body.plan,
            ),
          );
      } catch (error) {
  ...
  app.post(
    "/api/reports/action-plans/:planId/approve",
    reportOperator,
    async (request, response, next) => {
      if (!options.actionPlans || !options.approvedPlanExecutor) {
        response.status(503).json({ error: "reports_unavailable" });
        return;
      }
      try {
        ...
        if (!plan) {
          response.status(409).json({ error: "action_plan_not_draft" });
          return;
        }
        await options.approvedPlanExecutor.validate(user, plan.steps);
        response
          .status(200)
          .json(await options.actionPlans.approve(user, planId, body.revision));
      } catch (error) {
        next(error);
      }
    },
  ...
  app.post(
    "/api/reports/action-plans/:planId/regenerate",
    reportOperator,
    async (request, response, next) => {
      if (!options.actionPlans || !options.reportPlanning) {
        response.status(503).json({ error: "reports_unavailable" });
        return;
      }
      try {
        const plan = await options.actionPlans.findDraft(
          authenticatedUser(request),
          conversationIdSchema.parse(request.params.planId),
        );
        if (!plan) {
        ...
        response
          .status(200)
          .json(
            await options.actionPlans.updateSteps(
              authenticatedUser(request),
              plan.id,
              await options.reportPlanning.regenerate(plan.plan),
            ),
          );
      } catch (error) {
        next(error);
  ...
  app.post(
    "/api/reports/action-plans/:planId/execute",
    reportOperator,
    async (request, response, next) => {
      if (!options.actionPlans || !options.approvedPlanExecutor) {
        response.status(503).json({ error: "reports_unavailable" });
      ...
      try {
        const plan = await options.actionPlans.findApproved(
          authenticatedUser(request),
          conversationIdSchema.parse(request.params.planId),
        );
        if (!plan) {
        ...
        const outcome = await options.approvedPlanExecutor.execute(
          authenticatedUser(request),
          plan,
        );
        await options.actionPlans.recordExecution(
          authenticatedUser(request),
          plan.id,
          outcome,
        );
    ...
    async (request, response, next) => {
      ...
      try {
        ...
        response
          .status(200)
          .json(
            await options.reports.runNow(authenticatedUser(request), jobId),
          );
      } catch (error) {
        next(error);
      }
  ...
  app.use(
    (
      ...
      if (error instanceof ReportNotReadyError) {
        response.status(409).json({ error: "report_not_ready" });
        return;
      }
      ...
      if (error instanceof ActionPlanImmutableError) {
        response.status(409).json({ error: "action_plan_immutable" });
        return;
      }
      if (error instanceof ActionPlanValidationError) {
        response.status(422).json({ error: "action_plan_invalid" });
        return;
      }
      ...
      if (error instanceof IntegrationHealthSourceError) {
        response
          .status(502)
          .json({ error: `integration_health_source_${error.code}` });
        return;
      }
