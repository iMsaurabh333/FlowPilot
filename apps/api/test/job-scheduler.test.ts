...
describe("SAP Job Scheduling adapter", () => {
  ...
  it("creates a UTC one-time schedule with only the report-job identifier in callback data", async () => {
    const fetcher = vi.fn()
      ...
      .mockResolvedValueOnce(new Response(JSON.stringify({ _id: 42 }), { status: 201 }));
    ...
    expect(request.body).toContain('"name":"flowpilotreport11111111111141118111111111111111"');
  });

  it("accepts the scheduler job identifier returned in the Location header", async () => {
    const fetcher = vi.fn()
      .mockResolvedValueOnce(new Response(JSON.stringify({ access_token: "token" }), { status: 200 }))
      .mockResolvedValueOnce(new Response(null, { status: 201, headers: { Location: "/scheduler/jobs/42" } }));
    const client = new SapJobSchedulerClient({ url: "https://scheduler.example.test", uaa: { clientid: "client", clientsecret: "secret", url: "https://uaa.example.test" } }, fetcher);
